...
colors:
  ...
  violet-dark: "oklch(0.72 0.148 286)"
...
这个世界以浅色温润白瓷或深色玄曜星空为底，只让品牌强调色承担少量强调（当前服役预设为冷香紫罗兰，可插拔——冷香紫罗兰已废弃为设计基准，色值以色板生成器重跑为准）。层级主要来自字体重量、充足留白、轻边界、灰度头像和克制的表面明度差；小幅 spring 与悬停位移提供生命力，但不抢夺内容注意力。所有身份与发布信息来自站点配置或公开数据，缺失时收拢结构，不伪造头像、文案、指标或大块占位。
...
- 方言作用域入口统一为容器类名：公开内容 `.dialect-public`、工具 `.dialect-tool`、沉浸 `.dialect-immersive`（定义于 `web/src/styles/dialects/`）；`/lab/*` 不挂任何正式方言类。
...
- 仓库保留一条薄真实浏览器契约（`web/e2e/`，本地按需 `pnpm test:contract` 运行，不进 CI）：每种正式方言选择一个真实页面，验证计算后的 semantic token、主题首帧、关键控件状态和横向溢出；不维护整页像素快照基线。

### Visual Dialects

**视觉方言（Visual Dialect）**：同一套品牌基因在不同任务场景中的适配表达。全站共享色彩关系、排版尺度、圆角、边界和动效原则，但不共享固定布局与信息密度。

- **公开内容语言（Public Content Dialect）**：首页、博客、系列、图集、人设、友链、About 与登录／注册等公开入口强继承首页，以单一品牌强调色、近白／炭黑画布和宽松内容节奏为主。作用域入口 `.dialect-public`，由公开壳层容器统一挂载，Header、Footer 与公告条随之继承。
- **工具语言（Tool Dialect）**：后台、编辑器、设置、个人中心与聊天保留相同品牌基因，但采用更高信息密度、更明确的边界和完整语义状态。主要动作为高对比中性色，焦点环与当前导航等控件反馈用品牌强调。作用域入口 `.dialect-tool`，由壳层按路由挂载（`/profile`、`/chat`），后台布局独立挂载。
- **沉浸语言（Immersive Dialect）**：灯箱、媒体查看器与图表舞台使用中性舞台，让内容优先，只在控件反馈中克制地保留品牌色。作用域入口 `.dialect-immersive`。
