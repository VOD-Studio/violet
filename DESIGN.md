...
- **公开内容语言（Public Content Dialect）**：首页、博客、系列、图集、人设、友链、About 与登录／注册等公开入口强继承首页，以单一品牌强调色、近白／炭黑画布和宽松内容节奏为主。作用域入口 `.dialect-public`，由公开壳层容器统一挂载，Header、Footer 与公告条随之继承；`/chat` 在其工具方言迁移前保持根作用域。
- **工具语言（Tool Dialect）**：后台、编辑器、设置、个人中心与聊天保留相同品牌基因，但采用更高信息密度、更明确的边界和完整语义状态。作用域入口 `.dialect-tool`。
