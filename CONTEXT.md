...
**引用回复（Quoted Reply）**:
消息时间线中的一种新消息：引用一条同一会话内、发送者当前可见的文本或图片消息，并附带自己的文本或图片内容。引用不建立独立线程，允许引用另一条引用回复，但界面只展示一层引用预览；群聊系统事件不可作为引用目标。原消息被管理员删除后，引用块只显示“消息已删除”等占位，不保留或泄露原内容。引用回复沿用普通消息的不可变性、未读计数与通知规则，不新增特殊通知。
...
**图片消息（Image Message）**:
消息携带的图片内容。本期支持 JPEG、PNG、WebP、GIF，单张默认不超过 10 MB；不等同于任意文件附件。可选携带一段说明文字（caption），与图片同属一条消息，不再要求图片消息 content 必须为空；一次发送多张图片时仍各自一条独立消息，说明文字只跟随本轮发送的最后一张图片。

**消息不可变性（Message Immutability）**:
第一版消息发送后不可由发送者编辑或撤回；管理员可删除违规消息。该约束保持跨设备历史一致，避免“某设备看得到、另一设备看不到”的撤回语义。

**阅读位置（Read Position）**:
用户在每个会话中实际读到的最后一条消息位置。打开会话并读到最新可见消息后，未读计数清零；第一版不向其他成员展示已读回执。
...
**站内通知铃铛的聊天接入（Chat in Site Notifications）**:
新消息与房间邀请同时写入全站通知中心（来源类型 `chat_message` / `chat_room_invited`），与 Web Push 同一接收口径：排除发送者本人与已静音会话的成员，系统消息（成员变动等）不产生铃铛通知。同一会话只保留最新一条未读（新消息到达时旧未读折叠为已读，历史行保留），未读角标按会话而非按消息膨胀。用户在 /chat 读到最新消息后，铃铛里该会话的未读同步清除（聊天 MarkRead 发布领域事件，通知 subscriber 按来源置已读）。点击铃铛里的聊天通知跳转到对应会话；正在查看来源会话时新消息不再弹 toast。
...
_Avoid_: 已读回执（阅读位置的概念，与输入状态是两个独立信号）
