...
**笔记（Note）**:
轻量知识条目：markdown 正文（无长度上限）+ tags，标题可选。核心意图是「记录知识供未来检索」，读者可读但低打磨、诚实。**可编辑**（知识条目修正错漏、随时补充是常态，与推文的不可编辑形成领域级分界）、可删除。不进主博客文章流，也不进推文全局时间线。
...
**重量分诊（Weight Triage）**:
AI 会话结束时的沉淀分流，按会话产出的重量定去向：**功能级**（完整功能落地）→ 文章**草稿**，AI 产毛坯、人工打磨后发布；**经验级**（坑/根因/修法/教训）→ **笔记**，自动发布；**琐碎或重复** → 不沉淀。
