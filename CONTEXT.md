...
## 后台运维（Admin Operations）

> 本节定义 PRD-0027 的运维术语；新增能力按该 PRD 分期实施。

**运行日志（Runtime Log）**:
应用运行过程中产生的诊断记录，用于按时间、级别、来源和请求关联排查故障。允许按保留策略轮转，不承担用户行为追责的完整记录职责。
_Avoid_: 操作日志（会与审计混淆）、更新日志（指版本发布记录）

**操作审计（Operation Audit）**:
记录谁在何时对什么业务对象做了什么操作及其结果的追责事实。独立于运行日志，不能随运行日志清理策略删除。
_Avoid_: 运行日志、请求日志（均不能代替业务操作事实）

**素材引用（Media Reference）**:
内容对象或仍可恢复的内容版本对同一份素材的持有关系。工作稿、公开版本和可恢复历史分别构成保护依据，引用计数只是这些关系的汇总。

**孤儿素材候选（Orphan Media Candidate）**:
经过来源核对、暂未发现有效素材引用的正式素材。候选仍可能处于保护期或等待并发复核，不等于可以立即永久删除的文件。
_Avoid_: 孤儿分片（指临时上传文件）、零引用文件（不足以证明来源核对完成）

**备份集（Backup Set）**:
具备同一身份、恢复范围和一致性边界的数据库归档及可选素材包。仅数据库备份与包含素材的备份集具有不同恢复能力。
_Avoid_: 数据导出（未必包含恢复所需的结构与素材）、快照（与公开内容版本混淆）

**维护窗口（Maintenance Window）**:
为完成一致性备份或恢复等运维动作，暂停相关业务写入并处理在途写入的一段受控时期。维护结束以规定核对通过为条件，不以操作按钮返回为条件。

**短时运维授权（Operation Grant）**:
管理员完成二次身份验证后取得的、绑定当前会话和操作类别的限时授权。它不替代业务权限，也不延长或复活已失效的登录会话。
