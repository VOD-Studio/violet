...
**推文互动通知（Tweet Interaction Notification）**:
点赞、引用转发、评论、评论被回复四类互动进全站通知铃铛，接收者是**被互动内容的作者**——引用转发通知原推文作者而非转发者，回复通知被回复的评论作者。三条降噪规则：自我互动一律不通知；同一条评论同时命中「评论被回复」与「收到评论」且接收者同人时只发一条；取消点赞不通知，取消后再赞算新的一次互动。送达走两条通道：站内铃铛（SSE 实时 + toast）与浏览器 Web Push（见「浏览器通知」）。
_Avoid_: 消息（互动通知是低频事件提醒，不是会话消息，不进聊天未读）

## 笔记（Notes）

> 笔记是与文章、推文平级的独立内容类型，承载 AI 会话沉淀与轻量知识记录，不承担社交广播职责。

**笔记（Note）**:
轻量知识条目：markdown 正文（无长度上限）+ tags，标题可选。核心意图是「记录知识供未来检索」，读者可读但低打磨、诚实。**可编辑**（知识条目修正错漏、随时补充是常态，与推文的不可编辑形成领域级分界）、可删除，状态分**草稿/已发布**两态。不进主博客文章流，也不进推文全局时间线。
_Avoid_: 短文章（模型与发布意图都不同）、长推文（推文为社交而生且不可编辑）

**意图测试（Intent Test）**:
短内容归笔记还是推文的裁定规则，**按发布意图裁定、不按长度**：「记录知识、未来自己要搜」→ 笔记；「表达状态/情绪、期待互动」→ 推文。同一句话可以合法地出现在两边，但那是两个不同的动作。
_Avoid_: 按字数分档（长度是物理约束，不是意图判据）

**重量分诊（Weight Triage）**:
AI 会话收尾时的沉淀分流，按会话产出的重量定去向：**功能级**（完整功能落地）→ 文章**草稿**，AI 产毛坯、人工打磨后发布；**经验级**（坑/根因/修法/教训）→ **笔记**，去向经一键裁定；**琐碎或重复** → 不沉淀。
_Avoid_: 全量归档（未过滤的会话记录没有读者，也没有检索价值）

**一键裁定（One-click Disposition）**:
会话沉淀入库前的唯一人工环节：AI 完成分诊与成文后，以选项形式问一句「直接发布 / 存草稿 / 不发」，用户点一下即裁定去向，不要求先读全文。**公开可见性由这一次点击把关——AI 永不静默公开发布**。无应答（用户挂起、会话中断）时走安全默认：一律入库为草稿。文章毛坯恒为草稿（发布权不在 AI 的 PAT scope 内），不参与直发选项。
_Avoid_: 人审（审的是去向不是内容）、二次确认（仅一问，无连环弹窗）

**拆条（Per-insight Splitting）**:
一场会话的笔记产出粒度：每个独立知识点（一个坑/根因/修法/教训）单独成条，一场会话可产 0..N 条笔记 + 0..1 篇文章毛坯。一条笔记只装一个主题，tag 才打得准、检索才有效。
_Avoid_: 会话汇总条（多主题混装，tag 失准）

**自动化内容不进人类社交流**:
高频自动化内容源（AI 笔记）不进推文全局时间线等以「人声」为预期的信息流，否则时间线退化为机器人农场。与「聊天通知不进全站铃铛」同一条设计判断。
_Avoid_: 笔记同步到时间线（看似曝光增益，实为信息流污染）
...
## 人设档案（Persona Profiles）

> 人设档案承载站点公开展示的虚构角色设定，与作者资料和吉祥物实验相互独立。

**人设档案（Persona Profile）**:
一份可长期编辑的角色设定文档，由身份摘要、结构化资料项、Markdown 正文与有序设定图组成。后台可以维护多份，未被选择的档案不对访客公开。
_Avoid_: 作者档案（会与真实站点作者混淆）、吉祥物（后者属于动画引擎实验）、角色相册（只表达图片集合）

**当前人设（Active Persona）**:
站点唯一公开的人设选择关系，最多指向一份完整的人设档案。切换当前人设只替换选择关系，不改变或删除其他档案。
_Avoid_: 已发布人设（容易误解为多份同时公开）、默认人设（不能表达当前可切换的站点选择）

**人设资料项（Persona Fact）**:
档案速览中的一组有序 label/value 信息，用于表达生日、身高、身份等随角色变化的事实，不预设固定字段集合。
_Avoid_: 元数据（过于技术化）、标签（会与内容分类 Tag 混淆）

**设定图（Persona Image）**:
一张图片在某份人设档案中的编排位置，可带档案语境下的说明和无障碍文本；排在第一位的设定图同时承担公开页主视觉。
_Avoid_: 素材副本（档案只引用媒体素材）、独立封面（会产生第二套图片引用）
...
**提及（Mention）**:
消息正文里对某位会话成员的定向点名。正文存 `@(<username>:<user_id>)` 占位符，与自定义表情 `[name:uuid]`、内联图片 `![img:<media_id>]` 同为正文内联占位符，不建提及关系表——读时按占位符里的 user_id 解析被提及人当前资料（改名后旧消息仍指向同一人，占位符里的 username 只是解析不到该用户时的兜底文本）。渲染成可点的行内药丸，跳被提及人主页；被提及人是查看者本人时加深底色。只能提及该会话当前的有效成员，提及非成员的消息一律拒绝——正文不是向任意用户投递通知的通道。提及不产生独立的未读计数、不进全站通知铃铛，唯一的额外语义是绕过会话静音推送（见「聊天消息的通知通道」）；编辑可以改提及，但编辑不触发提及推送（只有新消息提醒）。
_Avoid_: 艾特（口语，不进文档与代码标识符）

**全体提及（Mention All）**:
房间内任意有效成员对发送时其他有效成员的一次集体点名，界面显示为「@所有人」，私聊不支持。它沿用提及的静音例外；重复全体提及或与个人提及重叠不重复提醒，编辑旧消息不产生新的提及提醒。
...
聊天与站内通知（含推文互动）**各自授权、各自订阅**：同一浏览器的同一 endpoint 可同时出现在两张订阅表里，关掉聊天提醒不影响通知铃铛的推送。两者共用一个 service worker——一个作用域只能注册一个，推送内容靠服务端下发的 title/url/tag 区分。

**聊天消息的通知通道（Chat Message Notification Channels）**:
新消息的提醒只走两个通道，不进全站通知铃铛（按消息逐条写通知会把铃铛列表刷爆）：一是顶栏聊天图标与浏览器标签页的未读提示（标签页显示未读总数，清零后恢复当前页面标题），聊天 SSE 事件流挂载在站点 Header（登录即建连），任意页面收到新消息角标即时刷新；二是浏览器 Web Push（需用户在聊天设置中授权）。铃铛仅保留房间邀请（`chat_room_invited`），邀请 toast 也由铃铛的 SSE 通道统一弹出。被提及者是静音的唯一例外：静音的语义是「别为日常消息吵我」而不是「别叫我」，被提及时仍推送，文案为专属的「有人提到了你」（未开启预览时不带正文）。
...
账号级自我装饰，三个维度：头像框、头像挂件（独立于头像框的右下角小饰件）、消息气泡主题；空值即原始样式。偏好只存素材目录 ID（应用层白名单校验，不接受 URL/CSS），外观跟人不跟消息——历史消息按发送者当前外观渲染，不把主题冻结在单条消息上。保存走版本号 CAS 乐观锁：冲突不静默覆盖，须显式重载后重选；重置也是一次显式保存（全空选择）。他人只能经会话列表/消息流的批量查询读到公开装饰字段，读不到版本号；其他设备经前台轮询与窗口聚焦重取新外观，不做即时推送。系统消息、已删除消息与推文分享卡片不受外观影响。
_Avoid_: 逐条消息外观（不冻结）、商城/付费解锁、用户自定义上传或 CSS

**聊天徽章（Chat Badge）**:
管理员授予的称号徽章，佩戴后以带字图片展示在消息发送者名旁。持有台账与佩戴选择分离：授予入 `chat_user_badges`（幂等、可撤销），佩戴（至多 3 枚、按序）存于聊天外观，读取时与持有集合求交集——撤销授予即自动从展示中消失，无需回写外观行。持有不设上限，佩戴才限 3。徽章目录是代码内固定白名单（24 枚，提供中文/日文/无字三版素材，站点渲染中文版），不接受任意图片。徽章是社区身份的可见信号（站长、初次相遇、阅读中……），不是成就系统：不由行为自动触发，只经管理员授予。
_Avoid_: 自动成就判定、商城/付费解锁、逐条消息外观冻结
...
**Bot API**:
面向持有 bot token 的外部程序的 HTTP 路由面（`/api/v1/chat/bot/*`），不绑定具体 bot 实现。入站：`GET /events`（SSE，订阅 bot 参与会话的事件）；出站：`POST .../messages`（发消息，返回消息 ID 供后续 Edit）、`PATCH /messages/{id}`（编辑消息，用于流式回复——先 Send 占位再逐步 Edit）、`POST .../typing`（输入状态）；查询：`GET .../conversations`、`GET .../messages`。管理路由 `/admin/chat/bots`（admin，需 `chat:bot-manage` 权限）。明文 token 仅在创建/重置时返回一次，丢失需重置。
...
**正文 / 附录**:
API 文档页的信息层级：公开端点为正文，`/admin/*` 管理端点为附录（默认折叠）。按路径前缀切分，不依赖手工维护的白名单。
_Avoid_: 公开分组 / 管理分组（未表达书册的正文附录隐喻）
