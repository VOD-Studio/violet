...
**图片消息（Image Message）**:
消息携带的图片内容。本期支持 JPEG、PNG、WebP、GIF，单张默认不超过 10 MB；不等同于任意文件附件。可选携带说明文字（caption），与图片同属一条消息，不再要求图片消息 content 必须为空；content 保留输入流中 `![img:<media_id>]` 占位符的位置，渲染端把占位符还原为与输入框同规格的内联图片（图文环绕，发出即所见）；一次发送多张图片时仍各自一条独立消息，各条携带输入流中环绕自身的文字。
