...
| 语言 | Go 1.25 |
| Web 框架 | chi v5 |
| ORM | GORM（全量 AutoMigrate） |
| 数据库 | PostgreSQL 16 |
| 缓存 | Redis 7（Token / 验证码存储） |
| 认证 | JWT（ES256，非对称签名） |
| 依赖注入 | google/wire（编译期，role/permission 模块）+ 手工装配（其余模块） |
| 邮件 | Resend API |
| 配置 | Viper（YAML + 环境变量） |
| 日志 | zerolog |
...
api/
├── cmd/server/            # 应用入口
│   ├── main.go            # 路由注册 + 依赖装配 + 启动
│   └── auth_adapter.go    # middleware 端口适配器（JWTService → TokenValidator）
├── config/                # 配置加载（Viper）
...
│   ├── infrastructure/    # 基础设施层：仓储实现、外部 API 适配器
│   ├── interfaces/http/   # 接口层：HTTP handler + 中间件
│   ├── middleware/        # 全局 HTTP 中间件（Auth/限流/CORS/审计）
│   ├── job/               # 定时任务（文件清理）
│   ├── migrate/           # 迁移执行器
│   └── service/           # 仅 emoji_seed_service.go（启动期 B站表情种子）
...
| **infrastructure** | 技术实现：数据库访问（GORM）、外部 API 调用、消息队列等。实现 domain 端口。 | 依赖 domain（实现端口） | `UserRepository` 的 GORM 实现 |
...
|------|--------|-------------|------|
| **auth** | user | auth/command + auth/query | 注册/登录/登出/令牌刷新/邮箱验证/密码重置 |
| **role** | role, permission | role + permission | 角色 CRUD、权限 CRUD、角色-权限分配 |
| **post** | post | post | 文章 CRUD、发布/归档/草稿状态机、浏览计数 |
| **comment** | comment | comment | 评论 CRUD、审核（通过/垃圾/删除）、批量操作 |
...
| **media** | upload, music | media | 文件详情/批量删除/缩略图 |
| **music** | music | media | 歌单管理、歌曲 CRUD、网易云/QQ音乐解析 |
...
|------|---------------------|------|------|
| `auth/` | — | JWTService, RedisTokenStore, RedisCodeStore | JWT 签发/校验、Token/验证码 Redis 存储 |
| `email/` | `EmailSender` | Sender (Resend) | 验证码/密码重置邮件 |
| `eventbus/` | `EventBus` | InMemory | 领域事件内存总线 |
| `github/` | `GitHubProvider` | Adapter | GitHub GraphQL + REST API |
| `music/` | `MusicProvider` | Provider | 网易云/QQ音乐解析（vkeys + meting 多源） |
...
func NewAuthContainer(db, redis, cfg, emailSender, bus) (*AuthContainer, error) {
    ...
    return &AuthContainer{AuthHandler: ..., JWTService: jwtService}, nil
}
```
...
make api-build    # 编译
make api-test     # 运行测试
make api-lint     # golangci-lint 检查
...
| `redis.*` | Redis 连接 |
| `jwt.*` | JWT 密钥路径与 Token TTL |
| `resend_api_key` | Resend 邮件 API Key |
| `superadmin.*` | 初始超级管理员账户 |
| `bilibili_cookie` | B站表情导入 Cookie |
