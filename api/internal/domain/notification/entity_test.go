...
func TestNewNotification_Valid(t *testing.T) {
	uid, sid, eid := newID(), newID(), newID()
	n, err := NewNotification(uid, eid, SourceCommentApproved, sid, "评论审核通过", "你的评论已审核通过", map[string]any{"post_title": "测试文章"})

	require.NoError(t, err)
	assert.Equal(t, uid, n.UserID())
	assert.Equal(t, eid, n.EventID())
...
func TestNewNotification_InvalidSourceType(t *testing.T) {
	_, err := NewNotification(newID(), newID(), SourceType("bogus"), newID(), "标题", "", nil)
	assert.ErrorIs(t, err, ErrInvalidSourceType)
}

func TestNewNotification_EmptyUserID(t *testing.T) {
	_, err := NewNotification(domainshared.ID{}, newID(), SourceCommentApproved, newID(), "标题", "", nil)
	assert.Error(t, err)
}

func TestNewNotification_EmptyEventID(t *testing.T) {
	_, err := NewNotification(newID(), domainshared.ID{}, SourceCommentApproved, newID(), "标题", "", nil)
	assert.Error(t, err)
}

func TestNewNotification_EmptySourceID(t *testing.T) {
	_, err := NewNotification(newID(), newID(), SourceCommentApproved, domainshared.ID{}, "标题", "", nil)
	assert.Error(t, err)
}

func TestNewNotification_EmptyTitle(t *testing.T) {
	_, err := NewNotification(newID(), newID(), SourceCommentApproved, newID(), "", "", nil)
	assert.Error(t, err)
}
...
func TestReconstruct(t *testing.T) {
	id, uid, sid, eid := newID(), newID(), newID(), newID()
	readAt := time.Now()
	created := time.Now().Add(-time.Hour)

	n := Reconstruct(id, uid, eid, SourceFriendLinkApplied, sid, "新友链申请", "收到新申请",
...
func TestIsValidSourceType(t *testing.T) {
	...
	assert.True(t, IsValidSourceType(SourceSubscriptionSucceeded))
...
func mustNew(t *testing.T) *Notification {
	t.Helper()
	n, err := NewNotification(newID(), newID(), SourceCommentApproved, newID(), "标题", "正文", nil)
	require.NoError(t, err)
	return n
}
