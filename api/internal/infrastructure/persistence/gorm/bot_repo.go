...
func (r *BotRepository) Save(ctx context.Context, bot *domainchat.Bot) error {
	...
	cols := []string{"user_id", "name", "avatar_id", "token_hash", "enabled", "show_thinking", "thinking_default_expanded", "updated_at"}
...
func (r *BotRepository) toPO(b *domainchat.Bot) (*model.ChatBot, error) {
	po := &model.ChatBot{
		...
		ThinkingDefaultExpanded: b.ThinkingDefaultExpanded(),
...
func (r *BotRepository) toDomain(po model.ChatBot) *domainchat.Bot {
	...
	return domainchat.ReconstructBot(
		...
		po.Name, avatarID, po.TokenHash, r.plainToken(po), po.Enabled, po.ShowThinking, po.ThinkingDefaultExpanded, po.CreatedAt, po.UpdatedAt,
	)
}
