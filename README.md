...
- **即时聊天**：登录用户之间的私聊与私有房间、文本/图片消息、全类型消息乐观发送（气泡内显示上传进度与失败重试，支持连续发送）、消息表情反应、@ 提及成员与房间内所有人（支持点击用户名提及成员，被提及时绕过会话静音）、持久化历史、SSE 实时事件、顶栏与浏览器标签页未读提示、浏览器 Web Push 通知、账号级聊天外观（头像框、头像挂件、消息气泡主题）
