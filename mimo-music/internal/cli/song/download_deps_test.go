...
func TestRunDownload_VIPNoSource(t *testing.T) {
	...
	err := runDownload(k, 99999, 1, t.TempDir(), false, deps)
	if err == nil {
...
func TestRunDownload_NilURL(t *testing.T) {
	...
	if err := runDownload(k, 1, 1, t.TempDir(), false, deps); err == nil {
		t.Fatal("nil Url 应报错")
	}
}
...
func TestRunDownload_MetadataFailureWarnf(t *testing.T) {
	...
	err := runDownload(k, 1, 1, dir, false, deps)
	if err != nil {
...
func TestRunDownload_UnwritableOut(t *testing.T) {
	...
	err := runDownload(k, 1, 1, target, false, deps)
	if err == nil {
		t.Fatal("不可写目录应报错")
	}
...
func TestRunDownload_OutMkdir(t *testing.T) {
	...
	if err := runDownload(k, 1, 1, nested, false, deps); err != nil {
...
func TestRunDownload_FLACFormat(t *testing.T) {
	...
	if err := runDownload(k, 1, 3, dir, false, deps); err != nil {
