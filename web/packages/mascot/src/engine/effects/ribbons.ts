...
/**
 * 自旋彩带:角速度阈值触发的 3D 倾斜轨道拖尾,跟随自旋、前后分层遮挡、
 * 惯性衰减回缩,承载旋转中段"无脸区"的可读性。
 */
export class RibbonFX implements Effect {
	...
	/**
	 * 清掉运动彩带:手动偏航检查表达静态姿态,不应把角度跳变误判成
	 * 真实旋转速度。
	 */
	clear(): void {
	...
	private spawnRibbon(lam0: number, dir: number): void {
		...
		this.createRibbon({
			o: {
				...
				rad: 110 + this.spawnIdx * tierStep + rand(-1.5, 1.5),
				radVel: rand(0, 2.5),
			...
			r: rw,
			hue: pl.baseHue + (360 * this.spawnIdx) / Math.max(pl.count, 1) + rand(-14, 14),
		});
		this.spawnIdx++;
	}
