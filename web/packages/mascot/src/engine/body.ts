/**
 * 身体几何 —— 软萌猫猫团 (Cat-Mochi) 轮廓与特征件系统。
 *
 * viewBox 0 0 260 260 坐标系：
 *   - 身体：软糯微扁的猫猫面团轮廓（上圆下阔，极富弹性）
 *   - 猫耳：左右一对圆润猫耳，含外耳廓与粉嫩内耳窝，支持独立支点旋转（飞机耳/立耳）
 *   - 尾巴：身体后侧圆润弯尾，随摆动参数与抬起高度自然摇曳
 *   - 小爪：胸前两只软萌肉垫爪爪，支持独立支点位移与旋转（挥手/踩奶/捂脸）
 *   - 嘴巴：多形态猫咪小嘴（经典 ω 嘴/开心张嘴/小圆哈欠/委屈撇嘴/吐舌）
 *   - 脸部：五官偏下半脸（y ≈ 142），呈现极致幼态萌感与柔和胡须
 */
...
/** 猫耳几何定义与旋转支点 (软萌微外倾猫耳，耳尖圆润倒角，耳根深扎防断层)。 */
export const CAT_EARS = {
	left: {
		pivot: [80, 88] as const,
		/** 外耳闭合路径：外侧优美外凸弧线，圆润耳尖，耳根圆盘深扎身体内部 */
...
/** 默认小波浪 ω 嘴常量 (向前兼容)。 */
export const MOUTH_PATH = MOUTH_SHAPES.w.lineD;
