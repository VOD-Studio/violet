...
/**
 * AnnouncementFeed - 首页公告区（事件日志方向）
 *
 * 公告实验室选型落地：事件日志方向（/lab/announcement）升为现役。
 * banner 形态在此排除——顶部 AnnouncementBar 已承担，事件流只收
 * card / article，避免同一条公告双曝光。排序权威是后端返回顺序
 * （sort_order ASC, created_at DESC），前端不重排。
 */
export default function AnnouncementFeed() {
	...
	if (isLoading) {
		return <AnnouncementSkeleton direction="log" />;
	}
