...
function getFileInfo(mimeType: string): { icon: typeof File; label: string } {
    ...
    if (mimeType.includes("zip")) return { icon: FileArchive, label: "ZIP 压缩包" };
    if (mimeType.includes("rar")) return { icon: FileArchive, label: "RAR 压缩包" };
    ...
    if (mimeType.startsWith("image/")) return { icon: ImageIcon, label: "图片文件" };
    return { icon: File, label: "文件" };
}

/**
 * 图片预览，支持缩略图占位、加载态和错误态
 * 缩略图立即显示，delay 结束后加载原图
 */
...
function VideoPreview({ url, mimeType }: { url: string; mimeType: string }) {
    return (
        <div className="bg-black">
            ...
            <video controls playsInline preload="metadata" className="max-h-125 w-full">
 ...
 * 音频预览，带图标装饰
 */
function AudioPreview({ url, mimeType, name }: { url: string; mimeType: string; name?: string }) {
    return (
        <div className="flex flex-col items-center gap-4 p-6">
            <div className="flex size-16 items-center justify-center rounded-full bg-primary/10">
                <Music className="size-8 text-primary" />
            </div>
            {name ? <p className="max-w-75 truncate text-sm font-medium">{name}</p> : null}
            {/* biome-ignore lint/a11y/useMediaCaption: 内部素材预览，无字幕需求 */}
    ...
    return (
        <div className={`space-y-3 ${className ?? ""}`} ref={ref}>
            ...
            <div className="overflow-hidden rounded-lg border bg-background">
                {isImage ? (
                    <ImagePreview url={url} thumbnailUrl={thumbnailUrl} name={name} delay={delay} />
                ) : null}
                {isVideo ? <VideoPreview url={url} mimeType={mimeType} /> : null}
                {isAudio ? <AudioPreview url={url} mimeType={mimeType} name={name} /> : null}
                {isPDF ? (
                    <iframe src={url} title={name ?? "PDF 预览"} className="h-150 w-full" />
                ) : null}
