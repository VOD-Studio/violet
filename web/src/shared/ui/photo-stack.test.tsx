...
describe("PhotoStack", () => {
	...
	it("两阶段拖拽：拉出至只剩 1/5 前 1:1 跟手置顶，过阈值后继续拖动平滑插回后置槽位且目标卡置顶", () => {
		...
		// 2. 超出 4/5 阈值继续向左拉 (280px > 224px) -> 目标卡置顶 (100)，当前卡钻入底层 (90) 并平滑向左槽位 (-10.64px) 插回
		...
		expect(Number(stack.getAttribute("data-current-offset"))).toBeCloseTo(-117.3, 1);

		// 3. 充分拖动 (336px = 280*1.2) -> 完全进入左侧后置槽位 (-10.64px)
		fireEvent.pointerMove(stack, { clientX: -36, pointerId: 1 });
		expect(Number(stack.getAttribute("data-current-offset"))).toBeCloseTo(-10.64, 1);
