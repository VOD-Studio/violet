...
description: Use when creating or editing an admin list/table page in web/src, or wiring any paged query. Covers hook choice (usePagedQuery vs useClientPagination), PageQuery type conventions, the backend paging contract, and the DataTable min-h-0 scroll chain.
---
...
| 需全量的下拉（如角色选择） | `useXxxAll` 模式：`limit=100` + `select` 解包 |

- 两个 hook 位于 `web/src/features/admin-shared/ui/data-table/hooks/`；新分页 hook 一律放各 feature 的 `hooks/` 目录。
...
- 唯一真相：`api/internal/domain/shared/paging.go` — `PageQuery{Page, Limit}`（`Normalize` 钳制 page≥1、1≤limit≤100，默认 20）+ `PageResult[T]{Items, Total, Page, Limit}`。
...
- admin 端点用 `RespondPaged`；公开端点（tags/projects）无分页参数时返回全量数组保持前台兼容，带参才 paged。
...
`DEFAULT_PAGE_SIZE = 50`（导出自 `use-paged-query.ts`）；posts 页保留 10。
